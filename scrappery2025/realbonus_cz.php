...
$baseUrl = 'https://www.realbonus.cz/realitni-kancelare/';
$baseDomain = 'https://www.realbonus.cz';
$maxPages = 9;

$data = [];

echo "<h2>realbonus.cz</h2>";
...
for ($page = 1; $page <= $maxPages; $page++) {
    ...
    foreach ($html->find('.small-perex-desc h2 a') as $link) {
        ...
        // Extrakce informací z tabulky
        foreach ($detailHtml->find('.blok table.table1 tr') as $row) {
            $th = $row->find('th', 0);
            $td = $row->find('td', 0);
            
            if ($th && $td) {
                $label = trim(strtolower($th->plaintext));
                $value = trim($td->plaintext);
                
                if (strpos($label, 'telefon') !== false) {
                    $telefon = $value;
                } elseif (strpos($label, 'e-mail') !== false || strpos($label, 'email') !== false) {
                    $email = $value;
                }
            }
        }
